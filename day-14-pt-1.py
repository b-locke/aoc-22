...
def modelingSand(caveData, unitsOfSand, source):
	...
	for grain in range(unitsOfSand):
		...
		while sandState == 'falling':
			if sandy < len(caveData) - 1:
				if caveCopy[sandy + 1][sandx] == '.':
						sandy += 1
				elif sandx > 0:
					if caveCopy[sandy + 1][sandx - 1] == '.':
						sandx -= 1
						sandy += 1
					elif sandx < len(caveData[0]) - 1:
						if caveCopy[sandy + 1][sandx + 1] == '.':
							sandx += 1
							sandy += 1
						else:
						...
						print('exceed right x', sandx, sandy, grainCount)
						return caveCopy
				else:
					print('exceed left x', sandx, sandy, grainCount)
					return caveCopy
			else:
				print('exceeded y length', len(caveData), sandx, sandy, grainCount)
				return caveCopy

	print(sandx, sandy, grainCount)
...
source = [500 - min([min([data[x][y][0] for y in range(len(data[x]))]) for x in range(len(data))]), 0]
detailedCave = addCaveDetails(cave, fillCoordinates(convertCoordinates(data)), source)
sandModeledCave= modelingSand(detailedCave, 1514, source)
