...
def hillClimbing(dataMap):
    start = [[[x, y] for y in range(len(dataMap[x])) if dataMap[x][y] == 'E'] for x in range(len(dataMap)) if 'S' in dataMap[x]][0][0]
    currentPosition = 'E'
    visitedList = [start]
    x = start[0]
    y = start[1]
    ...
    while currentPosition != 'a' and count < 10000:
        count += 1
        ...
        if x != 0 and [x - 1, y] not in visitedList:
            up = dataMap[x - 1][y]
            if alphaList.index(currentPosition) - alphaList.index(up) <= 1:
        ...
        if y != len(dataMap[0]) - 1 and [x, y + 1] not in visitedList:
            right = dataMap[x][y + 1]
            if alphaList.index(currentPosition) - alphaList.index(right) <= 1:
                newPath = currentPath.copy()
        ...
        if x != len(dataMap) - 1 and [x + 1, y] not in visitedList:
            down = dataMap[x + 1][y]
            if alphaList.index(currentPosition) - alphaList.index(down) <= 1:
                newPath = currentPath.copy()
                newPath.append([x + 1, y])
                pathList.append(newPath)
                visitedList.append([x + 1, y])
        ...
        if y != 0 and [x, y - 1] not in visitedList:
            left = dataMap[x][y - 1]
            if alphaList.index(currentPosition) - alphaList.index(left) <= 1:
                newPath = currentPath.copy()
                newPath.append([x, y - 1])
                pathList.append(newPath)
                visitedList.append([x, y - 1])
